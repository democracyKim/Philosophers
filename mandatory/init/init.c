...
/*   Updated: 2023/04/25 17:37:28 by minkim3          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/philo.h"

int	init_mutexes(t_resources *resources, int number_of_philosophers)
{
	int	i;
	...
	if (pthread_mutex_init(&resources->print, NULL) != 0)
		return (ERROR);
	if (pthread_mutex_init(&resources->time, NULL) != 0)
		return (ERROR);
	return (0);
}

int	init_philosophers(t_info *info, t_resources *resources, t_philo **philo)
{
	int	i;
